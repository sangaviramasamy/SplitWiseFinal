...
public class ExpenseServiceImplementation implements ExpenseService {
    ...
    public String validateUserSplit(ExpenseVo expenseVo){
        double percentage = 0;
    ...
    public Response addExpense(ExpenseVo expenseVo){
        ...
        for(UserExpenseMappingVo userExpense : expenseVo.getUserList()){
            if (!userValid.containsKey(userExpense.getUserId())) {
                response.setStatus("UserId " + userExpense.getUserId() + " , Not valid" );
                return response;
            }
        }
        if(!userValid.containsKey(expenseVo.getCreatedBy())){
            response.setStatus("UserId " + expenseVo.getCreatedBy()+ " , Not valid" );
            return response;
        }

        if(validateUserSplit(expenseVo).equals("Incorrect Split"))
        {
            response.setStatus("Incorrect Split");
            return response;
        }
        else if(expenseVo.getTotalAmount() <= 0){
            response.setStatus("Amount can only be positive");
            return response;
        }
        expenseVo.setExpenseId(generateUniqueId());
        for(UserExpenseMappingVo userExpense : expenseVo.getUserList()){
            if(userExpense.getSplitPercentage() <= 0){
                response.setStatus("Percentage can only be positive");
                return response;
            }
...
//        Map<Long , GroupPaymentSummary> childSummary = new HashMap<>();
//        Map<Long , Object> userGroup = new HashMap<>();
